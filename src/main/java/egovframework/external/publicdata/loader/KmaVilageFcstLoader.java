...
/**
 * 단기예보조회 적재 - {@code kcais.tb_ext_weather_vilage_fcst}. {@code KmaVilageFcstCleanser}
 * 산출물엔 facilityId가 없어(nx/ny만 있음) {@link RawStagingDto#getFacilityId()}에서 가져온다.
 *
 * <p>지점당 최대 ~72행(5일치)이 한 raw_staging 행에서 나온다 - {@code cleanse-db-schema-spec.md}
 * §2.4 명시대로 오늘+내일 필터는 여기서 하지 않고 수집된 전체 범위를 그대로 upsert한다.</p>
 *
...
public class KmaVilageFcstLoader implements PublicDataLoader {

    private static final String[] FIELDS =
        {"pop", "pty", "pcp", "reh", "sno", "sky", "tmp", "tmn", "tmx", "uuu", "vec", "vvv", "wav", "wsd"};

    private final WeatherVilageFcstMapper mapper;
