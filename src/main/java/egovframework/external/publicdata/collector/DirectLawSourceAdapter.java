 ...
 * 반영된다 - 실 API로 60건(현재 전체 대상) 전수 검증 완료(전부 성공).</p>
 *
 * <p><b>응답 형태 주의:</b> 기상청과 달리 resultCode/resultMsg 봉투가 없다. 성공하면
 * 최상위 키가 {@code "법령"}, 실패(존재하지 않는 MST 등)하면 {@code "Law"} 키에 에러 메시지
 * 문자열만 온다 - 실제 API로 두 경우 다 확인한 내용. 다만 {@code target=eflaw}에서 법령명이
 * 전혀 매칭 안 되는 경우는 이 JSON 에러 봉투 대신 HTML 에러 페이지("미신청된 목록/본문에
 * 대한 접근입니다")가 오는 것도 실측 확인 - 아래 {@link #validate}의 JSON 파싱 실패 처리
 * 경로가 이미 이 경우도 안전하게 {@link CollectException}으로 감싼다(추가 대응 불필요).</p>
 */
@Component
public class DirectLawSourceAdapter implements LawSourcePort {
    ...
    public String fetchLawBody(String sourceName, String apiName, String lawName) throws CollectException {
        ...
        String lm = URLEncoder.encode(lawName, StandardCharsets.UTF_8);
        String url = endpoint + "?OC=" + oc + "&target=eflaw&LM=" + lm + "&type=JSON";
        String responseBody;
        try {
        ...
        return validate(sourceName, apiName, lawName, responseBody);
    }

    private String validate(String sourceName, String apiName, String lawName, String responseBody) throws CollectException {
        try {
