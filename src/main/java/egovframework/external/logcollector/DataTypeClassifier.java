 ...
 * use_yn='N'으로 비활성화). {@code moleg-criminal-law}(법제처 형사법령정보)만 법령이고
 * 나머지(KMA 동네예보/기상특보/생활기상지수, safetydata 긴급재난문자)는 전부 공공데이터 -
 * 새 operationKey가 추가되면 기본값이 EXTERNAL_PUBLIC이 되도록 "법령 목록"만 관리한다
 * (반대로 "공공 목록"을 관리하면 새 오퍼레이션 추가 시 깜빡하고 안 넣었을 때 조용히
 * 누락되는 위험이 있음).</p>
 */
public final class DataTypeClassifier {
    ...
    private static final Set<String> LAW_OPERATION_KEYS = Set.of("moleg-criminal-law");
